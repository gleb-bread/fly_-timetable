...
export const ruleEmail = function () {
  ...
  const WORDS = initWords();
  return getRule(
    (value: string) => emailRegex.test(getString(value)) || WORDS.RULES.EMAIL
  );
};
...
export const ruleOnlyString = function () {
  ...
  return getRule(
    (value: string) => onlyLettersRegex.test(value) || WORDS.RULES.ONLYSTRING
  );
};
...
export const ruleEqualPasswords = function (password: string) {
  const WORDS = initWords();
  return getRule(
    (value: string) => value == password || WORDS.RULES.PASSWORDSNOTEQUAL
  );
};
